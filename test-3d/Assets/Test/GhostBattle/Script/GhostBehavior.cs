...
public class ExplosiveAttack : IGhostAttackBehavior
{
    ...
    public void Attack(GhostController ghost, Transform target)
    {
        ...
        if (!isExplode && distance <= ghost.Stat.data.attackRange)
        {
            isExplode = true;
            ghost.StartCoroutine(Explosive(ghost, target));
        }
    }

    public IEnumerator Explosive(GhostController ghost, Transform target)
    {
        ghost.OnExplosive();
...
public class RangedAttack : IGhostAttackBehavior
{
    ...
    {
        this.ghost = ghost;
        projectilePrefab = ghost.transform.Find("_VFX/_GhostProjectile").gameObject;
    }
...
public class SplitDamageReaction : IGhostDamageReaction
{
    ...
    private void SpawnMinorGhosts(GhostController ghost)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            GhostBattleManager.Instance.SpawnGhost(1005, ghost.transform.position);
        }
