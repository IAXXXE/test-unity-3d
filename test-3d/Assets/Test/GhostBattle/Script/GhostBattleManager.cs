...
{
    public static GhostBattleManager Instance;
    ...
    {
        foreach (var id in battleData.ghostIdList)
        {
            var ghostData = CharacterDatabase.Instance.GetGhostData(id);

            // 为每个类型创建独立的父对象
            Transform typeContainer = new GameObject($"Pool_{ghostData.ghostName}").transform;
            typeContainer.SetParent(poolContainer);

            // 创建该类型的对象池
            GhostTypePool pool = new GhostTypePool(
                ghostData.id,
                ghostData.worldPrefab,
                ghostData.maxSize,
                typeContainer
            );

            ghostPools[ghostData.id] = pool;
            
            Debug.Log($"Initialized ghost pool for {ghostData.ghostName} (ID: {ghostData.id}) with {ghostData.maxSize} instances");
        }
    }
    ...
    public IEnumerator StartBattle()
    {
        ...
        // 第一波在currPoint上方生成
        if (battleData.waveConfigs.Count > 0)
        {
            SpawnWaveByConfig(currPoint.position + Vector3.up * 2f, battleData.waveConfigs[0]);
            currentWave++;
        }
        ...
        while (battleTimer < battleData.totalBattleTime)
        {
            ...
            // 检查是否到达生成时间
            if (currentWave < battleData.waveConfigs.Count && 
                battleTimer >= currentWave * battleData.spawnInterval)
            {
                ...
                SpawnWaveByConfig(randomPoint, battleData.waveConfigs[currentWave]);
                currentWave++;
    ...
    /// 生成单个幽灵
    /// </summary>
    public void SpawnGhost(int ghostId, Vector3 centerPoint)
    {
